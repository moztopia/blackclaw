...
class ErrorResponse {
  ...
  static ErrorResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'message'), 'Required key "ErrorResponse[message]" is missing from JSON.');
        assert(json[r'message'] != null, 'Required key "ErrorResponse[message]" has a null value in JSON.');
        return true;
      }());
